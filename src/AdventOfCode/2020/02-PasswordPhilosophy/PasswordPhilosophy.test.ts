...
it("verifies an entry", () => {
    expect(verifyPasswordEntryAlternative({
        min: 1,
        max: 3,
        char: "a",
        pass: "abcde"
...
it("processes a list of passwords", () => {
    ...
    expect(validPasswords(exampleEntries, verifyPasswordEntryAlternative).length).toBe(1)
})
...
it("solves the problem", async () => {
    ...
    expect(validPasswords(inputArr, verifyPasswordEntryAlternative).length).toBe(588)
})